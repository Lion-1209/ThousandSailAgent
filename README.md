...
steps:
  - id: <步骤ID>              # 必填，唯一标识
    ...
    condition: <expression>    # 条件表达式，为 false 时跳过（可选）
```
...
步骤可通过 `condition` 字段控制是否执行。表达式为 `false` 时，步骤被跳过（status: skipped）。

**支持的操作：**

| 语法 | 说明 | 示例 |
|------|------|------|
| `==` / `!=` | 等于 / 不等于 | `input.env == "production"` |
| `contains` | 包含文本 | `steps.test.output contains "failed"` |
| `&&` | 与 | `input.env == "prod" && steps.test.status == "completed"` |
| `\|\|` | 或 | `steps.review.status == "failed" \|\| steps.test.status == "failed"` |
| `!` | 非 | `!(input.skip == "true")` |
| `()` | 分组 | `(a == "x") \|\| (b == "y")` |
...
|------|------|
| `input.xxx` | 运行时传入的参数 |
| `steps.xxx.status` | 上游步骤的状态（completed / failed / skipped） |
| `steps.xxx.output` | 上游步骤的输出文本 |
...
steps:
  ...
  - id: deploy_prod
    ...
    tools: [terminal]
    depends_on: [test]
    condition: 'input.env == "production" && steps.test.status == "completed"'
...
- [x] 中英文界面切换
- [x] 配置目录自动迁移
- [x] 条件分支（condition 表达式，运行时求值跳过步骤）
